...
class DataRepo:
    ...
    # -----------------------------------------------------------------------------
    def get_price_features(self, ticker: str, start=START_DATE, end=END_DATE, features: bool = True) -> pd.DataFrame:
        """
        ...
        rel_spread_oc = spread_oc / df['Close']
        rel_spread_hl = spread_hl / df['Close']
    ...
    # -----------------------------------------------------------------------------
    def get_fundamental_features(self, ticker: str, asof: Optional[str] = None) -> pd.DataFrame:
        """
        ...
        try:
            ...
            logger.info(f"[FUNDAMENTAL] Error for {ticker}: {e}")
            return pd.DataFrame()
    ...
    # -----------------------------------------------------------------------------
    def get_fred_macro_data(self, fred_series: Dict[str, str], start_date = START_DATE, end_date = END_DATE) -> pd.DataFrame:
        """
    ...
    def get_macro_features(self, ticker_map: Dict[str, str], start_date = START_DATE, end_date = END_DATE) -> pd.DataFrame:
        """
        ...
        for name, tkr in ticker_map.items():
            try:
                ...
                df = px[[price_col]].rename(columns={price_col: 'Close'})
    ...
    # Dataset builder
    # -----------------------------------------------------------------------------
    def build_quarterly_dataset(self, tickers: List[str], start_date=START_DATE, end_date=END_DATE) -> pd.DataFrame:
        """
        ...
        for t in tickers:
            ...
            row = self.get_fundamental_features(t)
        ...
        # 3) Macro features
        fred = self.get_fred_macro_data(FRED_DEFAULTS, start_date, end_date)
        macro_mkt = self.get_macro_features(MARKET_TICKERS, start_date, end_date)
        ...
        #return out
        if self.quarterly_data is None:
            self.quarterly_data = out
        else:
            self.quarterly_data = pd.concat([self.quarterly_data, out], ignore_index=True)
    ...
    def save_dataset(self):
        ...
        if self.quarterly_data is not None and not self.quarterly_data.empty:
